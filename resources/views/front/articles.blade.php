...
@section('title', 'Berita dan Artikel KAMCUP')

@section('content')

    <div class="container py-5 mt-5 pt-lg-0">
        <h2 class="fw-bold mb-4 text-center section-title-articles scroll-animate" data-animation="fadeInDown">Semua Artikel
            Seputar Olahraga & Komunitas</h2>

        <div class="mb-4 d-flex justify-content-end scroll-animate" data-animation="fadeInRight">
            <form action="{{ route('front.articles') }}" method="GET">
                <select name="filter" class="form-select border-primary-custom text-primary-custom"
                        ...
                        <div class="card h-100 border-0 shadow-sm card-hover-zoom overflow-hidden">
                            <div class="ratio ratio-4x3 article-thumbnail-wrapper">
                                <img src="{{ asset('storage/' . $article->thumbnail) }}"
                                    class="card-img-top object-fit-cover article-thumbnail" alt="{{ $article->title }}">
                            </div>
                            <div class="card-body">
        ...
        {{-- Custom Pagination --}}
        <div class="mt-4 d-flex justify-content-center scroll-animate" data-animation="fadeInUp">
            <nav aria-label="Page navigation for Articles">
                <ul class="pagination">
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ $articles->previousPageUrl() . '&filter=' . request('filter') }}"
                                rel="prev">&laquo;</a>
                        </li>
                    @endif
                    ...
                    @for ($i = $startPage; $i <= $endPage; $i++)
                        <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                            <a class="page-link"
                                href="{{ $articles->url($i) . '&filter=' . request('filter') }}">{{ $i }}</a>
                        </li>
                    @endfor
                    ...
                    @if ($articles->hasMorePages())
                        <li class="page-item">
                            <a class="page-link" href="{{ $articles->nextPageUrl() . '&filter=' . request('filter') }}"
                                rel="next">&raquo;</a>
                        </li>
                    @else
                        <li class="page-item disabled">
...
@push('styles')
    ...
    <style>
        ...
        :root {
            --primary-color: #002061;
            /* Biru Tua/Navy */
            --secondary-color: #fdbf06;
            /* Kuning Cerah/Emas */
            --accent-color: #c92788;
        ...
        .pagination {
            ...
            /* Matches `border` and `color` in your ref */
            --pagination-active-bg: var(--primary-color);
            /* Matches `background-color` and `border-color` in active state */
            --pagination-active-color: white;
            /* Text color for active page */
            --pagination-hover-bg: var(--secondary-color);
            /* Matches `background-color` and `border-color` on hover */
            --pagination-hover-color: white;
            /* Text color on hover */
            --pagination-border-color: var(--primary-color);
            /* Default border color */
            --pagination-disabled-color: rgba(0, 32, 97, 0.4);
            /* Matches `color` for disabled */
            --pagination-disabled-border: rgba(0, 32, 97, 0.2);
            /* Matches `border-color` for disabled */
        }
        ...
        .pagination .page-item.active .page-link {
            ...
            font-weight: bold;
            /* Make active page number bold */
            box-shadow: 0 4px 8px rgba(0, 32, 97, 0.2);
            /* Subtle shadow for active page (using primary-color with opacity) */
        }
        ...
        .pagination .page-item.disabled .page-link {
            ...
            background-color: transparent;
            /* Ensure disabled background is transparent */
            transform: none;
            /* Remove hover effect */
            box-shadow: none;
            /* Remove hover effect */
        ...
        @media (max-width: 575.98px) {
            .hero-section-articles {
                /* Not directly present, but good to keep if it exists elsewhere */
                padding-bottom: 2rem;
            }
        }
    </style>
